...
const getData = async () => {
  try {
    const response = await Api.get(`admin/yayasan`);
...
const doDeleteData = async (id, index) => {
  if (confirm("Apakah anda yakin menghapus data ini?")) {
    try {
      const response = await Api.delete(`admin/yayasan/${id}`);
        ...
        <div v-if="data">
          ...
          >
            <template #table-row="props">
              ...
              <span v-else-if="props.column.field == 'jml'">
                <div class="text-center">
                  {{ props.row.yayasandetail.length }}
                </div>
              </span>
