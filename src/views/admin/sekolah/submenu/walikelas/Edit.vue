...
const getDataId = async () => {
  try {
    const response = await Api.get(`admin/datasekolah/${id}/walikelas/${id2}`);
    dataDetail.value = {
...
const doStoreData = async (d) => {
  ...
  try {
    const response = await Api.put(
      `admin/datasekolah/${id}/walikelas/${id2}`,
      dataStore
    );
    Toast.success("Success", "Data Berhasil update!");
    // resetForm();
    router.push({ name: "AdminSekolahDetailWaliKelas", params: { id: id } });
...
<template>
  <div class="pt-4 px-10 md:flex justify-between">
    ...
    <div class="md:py-0 py-4 space-x-2 space-y-2">
      <router-link
        :to="{ name: 'AdminSekolahDetailWaliKelas', params: { id: id } }"
      >
        <button
