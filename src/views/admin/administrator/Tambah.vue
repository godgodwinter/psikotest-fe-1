...
const doStoreData = async (d) => {
  ...
  try {
    const response = await Api.post(`admin/administrator`, dataStore);
    Toast.success("Success", "Data Berhasil ditambahkan!");
    // resetForm();
    router.push({ name: "AdminAdministrator" });
    ...
    <div class="md:py-0 py-4">
      <BreadCrumb>
        <template v-slot:content>
          administrator <BreadCrumbSpace /> Tambah
        </template>
      </BreadCrumb>
    </div>
  </div>
  ...
  <div class="md:pt-6">
    <div class="md:flex justify-between px-10">
      ...
      <div class="space-x-1 space-y-1 pt-1 md:pt-0">
        <router-link :to="{ name: 'AdminAdministrator' }">
          <button
