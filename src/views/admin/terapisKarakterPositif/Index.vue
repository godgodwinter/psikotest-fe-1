...
const getData = async () => {
  try {
    const response = await Api.get(`admin/terapis`);
...
const doDeleteData = async (id, index) => {
  if (confirm("Apakah anda yakin menghapus data ini?")) {
    try {
      const response = await Api.delete(`admin/terapis/${id}`);
...
const doStoreDataImport = async (d) => {
  ...
  try {
    ...
    const response = await Api.post(`admin/proses/import/terapis`, formData, {
      headers: {
